...
//	custom k2 extra fields
define('K2_EXTRAFIELD_PDF_INDEX_FILE', 1);
define('K2_EXTRAFIELD_PDF_INDEX_DATA', 2);
	...
	function save($front = false)
	{
		...
		foreach ($variables as $key => $value)
		{
			if (( bool )JString::stristr($key, 'K2ExtraField_'))
			{
				...
				$objects[] = $object;
				if($object->id == K2_EXTRAFIELD_PDF_INDEX_FILE) {
					$file = JPATH_SITE . urldecode($object->value);
					$parser = new \Smalot\PdfParser\Parser();
					$pdf    = $parser->parseFile($file);
					$pdf_index_data = trim($pdf->getText());
				}
			}
		}

		//	if extra field id is PDF index, replace value.
		if(isset($objects)) {
			$loop = 0;
			foreach($objects as $object_rewrite) {
				if($object_rewrite->id == K2_EXTRAFIELD_PDF_INDEX_DATA) {
					$objects[$loop]->value = $pdf_index_data;
					unset($pdf_index_data);
				}
				$loop++;
			}
		}
